  ...
  if (err.errors) {
    errors = err.errors
      .map((error, idx) => {
        ...
        if (error.message === FEDERATED_ERROR.toString() && error.extensions) {
          return error.extensions.errors.map((err) =>
            formatError(toGraphQLError(err))
          );
        }
    ...
    if (
      ...
      if (errors.length === 1) {
        ...
        if (
          typeof err.errors[0].originalError !== 'undefined' &&
          typeof err.errors[0].originalError.statusCode === 'number'
        ) {
